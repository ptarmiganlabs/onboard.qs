...
flowchart TD
    ...
    D --> F[Fetch &#123;prefix&#125;/resources/autogenerated/<br/>product-info.js]
...
sequenceDiagram
    ...
    alt Already cached
        ...
        CM->>CM: getVirtualProxyPrefix()
        CM->>FS: fetch('{prefix}/resources/autogenerated/product-info.js')
...
When Qlik Sense is accessed through a virtual proxy (e.g. `https://server/finance/sense/app/…`), the fetch URL must include the virtual proxy prefix so the request is authenticated correctly. The helper `getVirtualProxyPrefix()` extracts the path segment(s) before `/sense/` from the current URL and prepends them to the resource path. When no virtual proxy is in use, the prefix is empty and the URL falls back to `/resources/autogenerated/product-info.js`.
