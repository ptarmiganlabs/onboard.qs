...
/**
 * Extract the virtual proxy prefix from the current URL.
 *
 * In client-managed Qlik Sense the URL pattern is
 * `https://host/[virtualproxy/]sense/app/…`.  Everything before the
 * `/sense/` segment is the virtual proxy prefix (which may be empty when no
 * virtual proxy is used).
 *
 * @returns {string} The virtual proxy prefix (e.g. "/finance"), or an empty
 *   string when no virtual proxy is present.
 */
export function getVirtualProxyPrefix() {
 ...
 *
 * The fetch URL is prefixed with the virtual proxy path (if any) so that
 * the request is routed through the same proxy as the Sense app itself.
 *
...
export async function getSenseVersion() {
    ...
    try {
        const prefix = getVirtualProxyPrefix();
        const resp = await fetch(`${prefix}/resources/autogenerated/product-info.js`);
        if (!resp.ok) {
